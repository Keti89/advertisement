	...
	{ 
		...
		if( $naslov=="" ||$podVid=="" || $cena=="" || $godina=="" || $opis=="" ||$ccnumber==""||$cctypes=="") 
		...
		else
			{
				...
				{
					$sql =mysqli_query($sConn, "INSERT INTO oglas (naslov,Vid,podVid,cena,godina,opis,id_korisnik, data_add) VALUES ('$naslov','Недвижнини','$podVid', '$cena', '$godina','$opis','$managerID',now())")or die(mysqli_error($sConn)); 
					$last_id = mysqli_insert_id($sConn);
					$exec = mysqli_query($sConn, $sql);
					$sql_2 =mysqli_query($sConn, "INSERT INTO nedviznini (sobi,sprat,km2,dvor,greenje,ido) VALUES ('$sobi','$sprat','$km2','$dvor','$greenje','$last_id')")or die(mysqli_error($sConn)); 
					$exec = mysqli_query($sConn, $sql_2);
				
					$img = $_FILES["image"]["name"];
					foreach ($img as $kei => $value) 
					{
						$photo= addslashes(file_get_contents($_FILES['image']['tmp_name'][$kei]));
						$image= getimagesize ($_FILES['image']['tmp_name'][$kei]);
				...
				$oglass .='<table width="50%" border="0" cellspacing="0"cellpadding="6" alert="center"><tr>';
				...
				$oglass .='</tr></table>';
			$oglass .="</br>Наслов: $naslov </br> Под вид: $podVid</br> Цена: $cena</br> Година: $godina </br> Соби: $sobi</br>Спрат: $sprat</br> Квадратура: $km2</br> Дворна поврсина: $dvor</br> Греење: $greenje</br> Опис: $opis</br>";
